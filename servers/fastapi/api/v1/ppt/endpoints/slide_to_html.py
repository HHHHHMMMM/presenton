...
async def generate_html_from_slide(
    ...
    """
    Generate HTML content from slide image and XML using OpenAI Chat Completions API with vision.
    ...
    """
    print(
        f"Generating HTML from slide image and XML using OpenAI Chat Completions API with vision..."
    )
    try:
        ...
        if not html_content:
            raise HTTPException(
                status_code=500, detail="No HTML content generated by OpenAI"
            )
...
async def generate_react_component_from_html(
    ...
    """
    Convert HTML content to TSX React component using OpenAI Chat Completions API.
    ...
    try:
        ...
        if not react_content:
            raise HTTPException(
                status_code=500, detail="No React component generated by OpenAI"
            )
...
async def edit_html_with_images(
    ...
    """
    Edit HTML content based on one or two images and a text prompt using OpenAI Chat Completions API.
    ...
    try:
        ...
        if not edited_html:
            raise HTTPException(
                status_code=500,
                detail="No edited HTML content generated by OpenAI",
            )
